...
(function() {
  const ReactObj = typeof React !== 'undefined' ? React : (typeof window !== 'undefined' && window.React ? window.React : this.React);
  const ReactDOMObj = typeof ReactDOM !== 'undefined' ? ReactDOM : (typeof window !== 'undefined' && window.ReactDOM ? window.ReactDOM : this.ReactDOM);
  const { useState, useEffect, useMemo, useContext, createContext, useCallback, useRef } = ReactObj;
  ...
  function getStored(key, fallback) {
    return safeGetStored(key, fallback);
  }

  function setStored(key, val) {
    safeSetStored(key, val);
  }

  // Auto-migration for updated departments & bookings
  (function initRoomVersion() {
    const currentVer = safeGetRaw('mplbp_ebilik_rooms_version');
    if (currentVer !== ROOMS_DATA_VERSION) {
      ...
      safeSetRaw('mplbp_ebilik_rooms_version', ROOMS_DATA_VERSION);
    }
  })();
  ...
  function App() {
    ...
    const [isLoggedIn, setIsLoggedIn] = useState(() => {
      return safeGetRaw('mplbp_ebilik_logged_in') === 'true';
    });
    const [currentUser, setCurrentUser] = useState(() => {
      const all = getStored('users', SEED_USERS);
      const uid = safeGetRaw('mplbp_ebilik_current_uid') || 'user-admin';
    ...
    const handleLoginSuccess = (userObj) => {
      ...
      safeSetRaw('mplbp_ebilik_current_uid', userObj.uid);
      safeSetRaw('mplbp_ebilik_logged_in', 'true');
      showToast('Log Masuk Berjaya', `Selamat datang, ${userObj.nama} (${ROLE_LABELS[userObj.role] || userObj.role})!`);
    };

    const handleLogout = () => {
      setIsLoggedIn(false);
      safeRemoveRaw('mplbp_ebilik_logged_in');
    ...
    const handleSwitchRole = (role) => {
      ...
      if (target) {
        ...
        safeSetRaw('mplbp_ebilik_current_uid', target.uid);
  ...
  function mountApp() {
    try {
      ...
      if (!el) return;
      const r = (typeof ReactDOM !== 'undefined' && ReactDOM.createRoot) ? ReactDOM : (typeof window !== 'undefined' && window.ReactDOM ? window.ReactDOM : null);
      if (r && r.createRoot) {
        const root = r.createRoot(el);
